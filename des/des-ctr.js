...
/**
 * DESCtr: Counter-mode (CTR) wrapper for Des.
 *
 * This encrypts a Unicode string to produces a base64 ciphertext using 128/192/256-bit Des,
 * and the converse to decrypt an encrypted ciphertext.
 *
 * See csrc.nist.gov/publications/detail/sp/800-38a/final
 */
class DESCtr extends Des {

    /**
     * Encrypt a text using Des encryption in Counter mode of operation.
     *
     ...
     *
     * @example
     *   const encr = DESCtr.encrypt('big secret', 'pāşšŵōřđ', 256); // 'lwGl66VVwVObKIr6of8HVqJr'
     */
    static encrypt(plaintext, password, nBits) {
        if (![ 128, 192, 256 ].includes(nBits)) throw new Error('Key size is not 128 / 192 / 256');
        plaintext = DESCtr.utf8Encode(String(plaintext));
        password = DESCtr.utf8Encode(String(password));

        // use Des itself to encrypt password to get cipher key (using plain password as source for key
        ...
        let key = Des.cipher(pwBytes, Des.keyExpansion(pwBytes)); // gives us 16-byte key
        ...
        const counterBlock = [ // 16-byte array; blocksize is fixed at 16 for Des
        ...
        // ------------ perform encryption ------------
        const ciphertextBytes = DESCtr.nistEncryption(plaintextBytes, key, counterBlock);
        ...
        const ciphertextB64 =  DESCtr.base64Encode(nonceStr+ciphertextUtf8);
    ...
    static nistEncryption(plaintext, key, counterBlock) {
        const blockSize = 16; // block size fixed at 16 bytes / 128 bits (Nb=4) for Des

        // generate key schedule - an expansion of the key into distinct Key Rounds for each round
        const keySchedule = Des.keyExpansion(key);
        ...
        for (let b=0; b<blockCount; b++) {
            ...
            const cipherCntr = Des.cipher(counterBlock, keySchedule);
    ...
    /**
     * Decrypt a text encrypted by Des in counter mode of operation.
     *
     ...
     *
     * @example
     *   const decr = DESCtr.decrypt('lwGl66VVwVObKIr6of8HVqJr', 'pāşšŵōřđ', 256); // 'big secret'
     */
    static decrypt(ciphertext, password, nBits) {
        if (![ 128, 192, 256 ].includes(nBits)) throw new Error('Key size is not 128 / 192 / 256');
        ciphertext = DESCtr.base64Decode(String(ciphertext));
        password = DESCtr.utf8Encode(String(password));

        // use Des to encrypt password (mirroring encrypt routine)
        const nBytes = nBits/8; // no bytes in key
        const pwBytes = new Array(nBytes);
        for (let i=0; i<nBytes; i++) { // use 1st nBytes chars of password for key
            pwBytes[i] = i<password.length ?  password.charCodeAt(i) : 0;
        }
        let key = Des.cipher(pwBytes, Des.keyExpansion(pwBytes));
        key = key.concat(key.slice(0, nBytes-16)); // expand key to 16/24/32 bytes long
        ...
        // ------------ perform decryption ------------
        const plaintextBytes = DESCtr.nistDecryption(ciphertextBytes, key, counterBlock);
        ...
        const plaintext = DESCtr.utf8Decode(plaintextUtf8);
